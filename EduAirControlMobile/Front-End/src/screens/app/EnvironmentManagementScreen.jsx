...
import { STATUS_LABELS } from "../../constants/environments";
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function getMetricColor(key, val, currentColors) {
  if (key === "temp")
    return val > 26
      ? currentColors.error
      : val > 24
...
function FormField({
  ...
  return (
    <View style={styles.fieldGroup}>
      <Text style={[styles.fieldLabel, { color: currentColors.textMuted }]}>
        {label}
      </Text>
      <TextInput
        style={[
  ...
  return (
    ...
    >
      ...
      <Text
        style={[
          styles.summaryValue,
          { color: active ? accent : currentColors.textPrimary },
        ]}
      >
        {value}
      </Text>
      <Text
        style={[
          styles.summaryLabel,
          { color: active ? accent : currentColors.textMuted },
        ]}
      >
        {label}
      </Text>
  ...
  const statusColor =
    environment.statusKey === "normal"
      ...
      : environment.statusKey === "warning"
        ...
        : currentColors.error;
  const statusLabel =
    t(`status.${environment.statusKey}`) ||
  ...
  return (
    ...
    >
      ...
      <View style={styles.cardTop}>
        <View
          style={[styles.cardIcon, { backgroundColor: `${statusColor}18` }]}
        >
      ...
      >
        {metrics.map((m) => {
          const color = getMetricColor(m.key, raws[m.key], currentColors);
          return (
      ...
      >
        ...
        >
          ...
          />
          <Text style={[styles.editBtnTxt, { color: currentColors.accent }]}>
            {t("common.edit")}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          ...
          />
          <Text style={[styles.deleteBtnTxt, { color: currentColors.error }]}>
            {t("common.delete")}
          </Text>
        </TouchableOpacity>
      </View>
...
export default function EnvironmentManagementScreen({ navigation }) {
  ...
  const stats = useMemo(
    () => ({
      ...
      normals: environments.filter(isNormalEnvironment).length,
      warnings: environments.filter(isWarningEnvironment).length,
      alerts: environments.filter(isAlertEnvironment).length,
    }),
    [environments],
  );
  ...
  const filtered = useMemo(() => {
    ...
    const list = environments.filter((env) => {
      const capacity = Number(env.capacity) || 0;
      const name = String(env.name || "").toLowerCase();
      const location = String(env.location || "").toLowerCase();
      const matchSearch =
        !query || name.includes(query) || location.includes(query);
      const matchFilter =
        activeFilter === "all" ||
        (activeFilter === "normal" && isNormalEnvironment(env)) ||
        (activeFilter === "warning" && isWarningEnvironment(env)) ||
        (activeFilter === "alert" && isAlertEnvironment(env));
      const matchMin = min === null || capacity >= min;
      const matchMax = max === null || capacity <= max;
    ...
    return [...list].sort((a, b) => {
      ...
      if (sortBy === "status") {
        return (
          (statusOrder[getEnvironmentStatusKey(a)] ?? 3) -
          (statusOrder[getEnvironmentStatusKey(b)] ?? 3)
        );
      }
      return String(a.name || "").localeCompare(String(b.name || ""));
    });
  }, [activeFilter, environments, maxCapacity, minCapacity, search, sortBy]);
    ...
    >
      ...
      >
        ...
        >
          ...
          <Text style={[styles.addBtnTxt, { color: currentColors.bgBody }]}>
            {t("management.add")}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={styles.summaryRow}>
          ...
          <SummaryCard
            ...
            value={stats.normals}
            emoji="✅"
            accent={currentColors.success}
            active={activeFilter === "normal"}
            onPress={() => setActiveFilter("normal")}
            currentColors={currentColors}
          />
          <SummaryCard
            label={t("management.warning")}
            value={stats.warnings}
            emoji="🔔"
            accent={currentColors.warning}
        ...
        <View style={styles.controlsRow}>
          ...
          >
            ...
            <TextInput
              ...
              placeholder={t("management.min")}
              placeholderTextColor={currentColors.textMuted}
              value={minCapacity}
              onChangeText={(value) =>
                setMinCapacity(value.replace(/[^\d]/g, ""))
              }
              keyboardType="numeric"
            />
            <Text
              style={[styles.controlSep, { color: currentColors.textMuted }]}
            >
              -
            </Text>
            <TextInput
              ...
              placeholder={t("management.max")}
              placeholderTextColor={currentColors.textMuted}
              value={maxCapacity}
              onChangeText={(value) =>
                setMaxCapacity(value.replace(/[^\d]/g, ""))
              }
              keyboardType="numeric"
            />
          </View>
          ...
          >
            ...
            >
              {t(`management.sort${sortBy[0].toUpperCase()}${sortBy.slice(1)}`)}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.resultsInfo}>
          <Text
            style={[styles.resultsCount, { color: currentColors.textMuted }]}
          >
          ...
          {(search ||
            ...
            <TouchableOpacity onPress={clearAll}>
              <Text style={[styles.clearTxt, { color: currentColors.accent }]}>
                {t("management.clear")}
              </Text>
            </TouchableOpacity>
          )}
        ...
        {filtered.length === 0 ? (
          <View style={styles.empty}>
            ...
            {search || activeFilter !== "all" ? (
              ...
              >
                <Text
                  style={[styles.emptyBtnTxt, { color: currentColors.bgBody }]}
                >
                  {t("management.clearSearch")}
                </Text>
              </TouchableOpacity>
            ) : (
              <TouchableOpacity
      ...
      >
        ...
        >
          ...
          >
            ...
            {[
              { key: "name", label: t("management.sortName") },
              { key: "capacity", label: t("management.sortCapacity") },
              { key: "status", label: t("management.sortStatus") },
            ].map((option) => (
              <TouchableOpacity
                key={option.key}
                style={[
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={styles.modalHeader}>
              <Ionicons
                name={
                  modal.mode === "add" ? "add-circle-outline" : "create-outline"
                }
                size={22}
                color={currentColors.accent}
              />
              ...
              >
                {modal.mode === "add"
                  ? t("management.newEnvironment")
                  : t("management.editEnvironment")}
              </Text>
              <TouchableOpacity
                onPress={closeModal}
            ...
            <View style={styles.modalActions}>
              ...
              >
                <Text
                  style={[styles.saveBtnTxt, { color: currentColors.bgBody }]}
                >
