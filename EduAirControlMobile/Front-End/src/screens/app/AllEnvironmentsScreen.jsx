...
function getStatusKey(env) {
  if (isNormal(env)) return 'normal'
  if (isWarning(env)) return 'warning'
  return 'alert'
}
...
function EnvironmentRow({ env, currentColors, t, onPress }) {
  ...
  return (
    <TouchableOpacity
      style={[styles.card, { backgroundColor: currentColors.bgCard, borderColor: currentColors.borderColor, borderLeftColor: statusColor }]}
      onPress={onPress}
      activeOpacity={0.85}
    >
      ...
      <View style={styles.cardInfo}>
        ...
        <View style={styles.metaRow}>
          ...
          <Text style={[styles.metaText, { color: currentColors.textMuted }]}>{env.capacity}</Text>
        </View>
      </View>
      <View style={styles.rightSide}>
        <View style={[styles.scorePill, { borderColor: statusColor }]}>
          <Text style={[styles.scoreText, { color: statusColor }]}>{calcScore(env)}</Text>
        </View>
        <Text style={[styles.statusText, { color: statusColor }]}>{t(`status.${statusKey}`)}</Text>
      </View>
    </TouchableOpacity>
  )
}
  ...
  if (!loaded) {
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: currentColors.bgBody }]}>
        <View style={styles.center}><Text style={{ color: currentColors.textMuted }}>{t('loading')}</Text></View>
      </SafeAreaView>
    )
  }
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: currentColors.bgBody }]}>
      ...
      <View style={[styles.header, { backgroundColor: currentColors.bgCard, borderBottomColor: currentColors.borderColor }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.headerTitle, { color: currentColors.textPrimary }]}>{t('environments.title')}</Text>
          <Text style={[styles.headerSub, { color: currentColors.textMuted }]}>{t('environments.subtitle')}</Text>
        </View>
        <NotificationButton />
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.controlsRow}>
          ...
          >
            ...
            <Text style={[styles.filterButtonText, { color: currentColors.textPrimary }]}>{t('environments.filters')}</Text>
            {activeFiltersCount > 0 && (
              <View style={[styles.filterBadge, { backgroundColor: currentColors.accent }]}>
                <Text style={[styles.filterBadgeText, { color: currentColors.bgBody }]}>{activeFiltersCount}</Text>
              </View>
            )}
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            ...
            <Text style={[styles.quickSortText, { color: currentColors.textSecondary }]}>{sortFilters.find((item) => item.key === sort)?.label}</Text>
          </TouchableOpacity>
        </View>

        <Text style={[styles.resultCount, { color: currentColors.textMuted }]}>
          {t('management.showing', { shown: filtered.length, total: environments.length })}
        </Text>

        {filtered.length === 0 ? (
          <View style={styles.empty}>
            <Ionicons name="search-outline" size={56} color={currentColors.borderColor} />
            <Text style={[styles.emptyTitle, { color: currentColors.textPrimary }]}>{t('management.noResults')}</Text>
            <Text style={[styles.emptyText, { color: currentColors.textMuted }]}>{t('management.tryAnother')}</Text>
          </View>
        ) : filtered.map((env) => (
          <EnvironmentRow
            key={env.id}
      ...
      <Modal visible={filtersOpen} transparent animationType="fade" onRequestClose={() => setFiltersOpen(false)}>
        <View style={styles.modalOverlay}>
          ...
          <View style={[styles.filterSheet, { backgroundColor: currentColors.bgBody }]}>
            <TouchableOpacity style={styles.closeBtn} onPress={() => setFiltersOpen(false)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            ...
            <View style={[styles.sheetFooter, { backgroundColor: currentColors.bgCard, borderTopColor: currentColors.borderColor }]}>
              <TouchableOpacity style={styles.clearButton} onPress={clearFilters}>
                <Text style={[styles.clearButtonText, { color: currentColors.accent }]}>{t('environments.clearFilters')}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.resultsButton, { backgroundColor: currentColors.accent }]}
                onPress={() => setFiltersOpen(false)}
              >
                <Text style={[styles.resultsButtonText, { color: currentColors.bgBody }]}>
...
const styles = StyleSheet.create({
  ...
  sheetBody: { flex: 1, flexDirection: 'row', overflow: 'hidden', borderTopLeftRadius: 20, borderTopRightRadius: 20 },
  sectionMenu: {
    width: 135,
    borderRightWidth: 1,
  },
