...
import { IDEAL_RANGES } from "../../constants/environments";
...
function getTranslatedIssueDetail(metric, t) {
  if (metric.key === "temp") {
    return metric.raw < 18
      ? t("environmentDetail.tempLow", {
          value: metric.raw,
          ideal: metric.ideal,
        })
      : t("environmentDetail.tempHigh", {
          value: metric.raw,
          ideal: metric.ideal,
        });
  }
  if (metric.key === "humidity") {
    return metric.raw < 40
      ? t("environmentDetail.humidityLow", {
          value: metric.raw,
          ideal: metric.ideal,
        })
      : t("environmentDetail.humidityHigh", {
          value: metric.raw,
          ideal: metric.ideal,
        });
  }
  if (metric.key === "co2") {
    return t("environmentDetail.co2High", {
      value: metric.raw,
      ideal: metric.ideal,
    });
  }
  if (metric.key === "noise") {
    return t("environmentDetail.noiseHigh", {
      value: metric.raw,
      ideal: metric.ideal,
    });
  }
  ...
  const rows = [
    {
      ...
      label: "Temp",
      value: `${latest.temp}°C`,
      delta: `${trend.temp > 0 ? "+" : ""}${trend.temp}°C`,
    },
    {
      key: "humidity",
      label: t("environmentDetail.humidity"),
      value: `${latest.humidity}%`,
      delta: `${trend.humidity > 0 ? "+" : ""}${trend.humidity}%`,
    },
    {
      key: "co2",
      label: "CO2",
      value: `${latest.co2} ppm`,
      delta: `${trend.co2 > 0 ? "+" : ""}${trend.co2}`,
    },
    {
      key: "noise",
      label: t("environmentDetail.noise"),
      value: `${latest.noise} dB`,
      delta: `${trend.noise > 0 ? "+" : ""}${trend.noise}`,
    },
  ];
    ...
    >
      <View style={styles.historyTimeline}>
        {history.map((item) => (
          <View key={item.time} style={styles.historyPoint}>
            ...
            />
            <Text
              style={[styles.historyTime, { color: currentColors.textMuted }]}
            >
              {item.time}
            </Text>
          </View>
      ...
      <View style={styles.historyRows}>
        {rows.map((row) => (
          ...
          >
            <Text
              style={[styles.historyLabel, { color: currentColors.textMuted }]}
            >
              {row.label}
            </Text>
            <Text
  ...
  return (
    ...
    >
      <View style={styles.metricTileTop}>
        ...
        <Text style={[styles.metricLabel, { color: currentColors.textMuted }]}>
          {metric.label}
        </Text>
      </View>
      ...
      <Text style={[styles.metricValue, { color: currentColors.textPrimary }]}>
        {metric.value}
        <Text style={[styles.metricUnit, { color: currentColors.textMuted }]}>
          {metric.unit}
        </Text>
      </Text>
      ...
      <View style={styles.metricFooter}>
        <Text
          style={[styles.idealText, { color: currentColors.textMuted }]}
          numberOfLines={1}
        >
          {t("environmentDetail.ideal", { range: metric.ideal })}
        </Text>
        {metric.warning && (
          <Ionicons name="alert-circle" size={14} color={metric.color} />
        )}
      </View>
    </View>
  );
  ...
  return (
    ...
    >
      <View style={styles.sectionHeader}>
        ...
        >
          {t("environmentDetail.perceivedComfort")}
        </Text>
      </View>
      ...
      <TouchableOpacity
        style={[
          ...
          {
            backgroundColor:
              rating > 0 ? currentColors.accent : currentColors.borderColor,
          },
        ]}
        onPress={() => rating > 0 && onSubmit(rating)}
        disabled={rating === 0}
      >
        <Text style={[styles.ratingBtnTxt, { color: currentColors.bgBody }]}>
          {t("environmentDetail.submitRating")}
        </Text>
      </TouchableOpacity>
    </View>
  );
...
export default function EnvironmentDetailScreen({ route, navigation }) {
  ...
  const score = env ? calcEnvironmentScore(env) : 0;
  const scoreColor =
    score >= 75
      ? currentColors.success
      : score >= 50
  ...
  if (!env) {
    return (
      ...
      >
        <View style={styles.center}>
          <Text style={{ color: currentColors.textPrimary, fontSize: 16 }}>
            {t("environmentDetail.notFound")}
          </Text>
          <TouchableOpacity onPress={() => navigation.goBack()}>
            <Text style={{ color: currentColors.accent, marginTop: 10 }}>
              {t("environmentDetail.back")}
            </Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
  ...
  const metrics = [
    {
      ...
      color:
        temp < 18 || temp > 24 ? currentColors.warning : currentColors.success,
    ...
    },
    {
      ...
      color: co2 > 1000 ? currentColors.error : currentColors.success,
      warning: co2 > 1000,
      min: 400,
      max: 2000,
    },
    {
      ...
      color: noise > 50 ? currentColors.warning : currentColors.success,
      warning: noise > 50,
      min: 0,
      max: 120,
    },
  ...
  const recommendation = activeIssues.length
    ? t("environmentDetail.recommendationReview", {
        ...
        >
          <View style={styles.heroTop}>
            <View
              style={[
                styles.roomIcon,
                { backgroundColor: statusDim, borderColor: statusColor },
              ]}
            >
            ...
            <View style={styles.heroTitleBlock}>
              <Text
                style={[styles.roomName, { color: currentColors.textPrimary }]}
                numberOfLines={2}
              >
                {env.name}
              </Text>
          ...
          <View style={styles.badgeRow}>
            ...
            >
              <Ionicons
                name={STATUS_ICONS[env.statusKey] || "help-circle"}
                size={14}
                color={statusColor}
              />
            ...
            >
              ...
              <Text style={[styles.statusBadgeText, { color: qualityColor }]}>
                {qualityLabel}
              </Text>
            </View>
            <View
            ...
            >
              ...
              <Text style={[styles.statusBadgeText, { color: scoreColor }]}>
                {t("environmentDetail.score")}: {score}
              </Text>
            </View>
          </View>
          ...
          >
            ...
            />
            <View style={styles.heroStat}>
              <Text style={[styles.heroStatValue, { color: qualityColor }]}>
                {qualityLabel}
              </Text>
              <Text
                style={[
...
const styles = StyleSheet.create({
  ...
  roomName: { fontSize: 21, fontWeight: "900", lineHeight: 27 },
